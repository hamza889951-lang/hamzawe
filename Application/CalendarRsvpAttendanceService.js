...
const CalendarRsvpAttendanceService = {
  ...
  initializeBaseline() {
    ...
    if (active.data) {
      return Result.fail('RSVP_ACTIVE_REQUIRES_DEACTIVATION', 'Calendar RSVP baseline cannot be initialized while active');
    }

    const initialized = CalendarRsvpConfigRepository.getInitialized();
    if (!initialized.ok) return initialized;
    const binding = CalendarRsvpConfigRepository.validateConfigBinding(ready.data, false);
    if (!binding.ok) return binding;
    if (initialized.data && binding.data.matches) {
      return Result.fail('RSVP_BASELINE_ALREADY_INITIALIZED', 'Calendar RSVP baseline is already initialized for the current configuration');
    }

    return this._runSync(ready.data, {
      baseline: true,
      resetCheckpoints: !!initialized.data && !binding.data.matches
    });
  },
  ...
  syncNow() {
    ...
    const binding = CalendarRsvpConfigRepository.validateConfigBinding(ready.data, true);
    if (!binding.ok) return binding;
    const initialized = CalendarRsvpConfigRepository.getInitialized();
    if (!initialized.ok) return initialized;
    ...
    const trusted = CalendarRsvpConfigRepository.validateTrustedOperator(ready.data);
  ...
  activate() {
    ...
    const binding = CalendarRsvpConfigRepository.validateConfigBinding(ready.data, false);
    if (!binding.ok) return binding;
    if (!binding.data.matches) {
      return Result.fail('RSVP_REBASELINE_REQUIRED', 'Current Calendar RSVP configuration differs from the last baseline; reinitialize while inactive before activation', {
        configuredCalendarId: ready.data.calendarId,
        configuredSecretaryEmail: ready.data.secretaryEmail,
        binding: binding.data.binding || null
      });
    }

    const trusted = CalendarRsvpConfigRepository.validateTrustedOperator(ready.data);
    if (!trusted.ok) return trusted;
    const triggerResult = CalendarRsvpTrigger.install(ready.data.calendarId);
    if (!triggerResult.ok) return triggerResult;
    return CalendarRsvpConfigRepository.setActive(true);
  },

  deactivate() {
    return CalendarRsvpConfigRepository.setActive(false);
  },
  ...
  _runSyncLocked(config, options) {
    ...
    while (true) {
      ...
      if (options.resetCheckpoints) {
        const clearCheckpoints = CalendarRsvpConfigRepository.clearCheckpoints();
        if (!clearCheckpoints.ok) return clearCheckpoints;
      }

      const correlation = this._buildCorrelationIndex();
      if (!correlation.ok) return correlation;

      const processed = this._processEvents(pageResult.data.items, config, !!options.baseline, correlation.data);
      if (!processed.ok) return processed;

      const save = CalendarRsvpConfigRepository.saveState(pageResult.data.nextSyncToken, true, config);
      ...
      return Result.ok({
        ...
        unmatchedEvents: processed.data.unmatchedEvents,
        ambiguousEvents: processed.data.ambiguousEvents,
  ...
  _buildCorrelationIndex() {
    if (typeof SlotRepository === 'undefined' || !SlotRepository.queryResult) {
      return Result.fail('RSVP_AVAILABILITY_SOURCE_UNAVAILABLE', 'SlotRepository.queryResult is required for RSVP correlation');
    }
    const result = SlotRepository.queryResult(function() { return true; });
    if (!result.ok) {
      return Result.fail('RSVP_AVAILABILITY_SOURCE_UNAVAILABLE', 'Failed to read Availability for RSVP correlation', result.error);
    }
    const index = Object.create(null);
    const rows = Array.isArray(result.data) ? result.data : [];
    for (let i = 0; i < rows.length; i++) {
      const uid = rows[i] && typeof rows[i].calendar_event_id === 'string'
        ? rows[i].calendar_event_id.trim()
        : '';
      if (!uid) continue;
      if (!index[uid]) index[uid] = [];
      index[uid].push(rows[i]);
    }
  ...
  _collectPages(calendarId, syncToken) {
    ...
    while (true) {
      ...
      if (!result.data.nextSyncToken) {
        return Result.fail('RSVP_SYNC_TOKEN_MISSING', 'Final Calendar sync page did not return nextSyncToken');
      }
  ...
  _processEvents(items, config, baseline, correlationIndex) {
    ...
    for (let i = 0; i < items.length; i++) {
      processedEvents++;
      const outcome = this._processEvent(items[i] || {}, config, baseline, correlationIndex);
      ...
      unmatchedEvents += outcome.data.unmatched ? 1 : 0;
      ambiguousEvents += outcome.data.ambiguous ? 1 : 0;
    }
  ...
  _processEvent(event, config, baseline, correlationIndex) {
    ...
    if (event.status === 'cancelled') {
      return this._saveCheckpoint(key, {
        responseStatus: '',
        eventStatus: 'cancelled',
        outcome: this.RECONCILIATION.IGNORED_CANCELLED,
    ...
    const previous = CalendarRsvpConfigRepository.getCheckpoint(key);
    if (!previous.ok) return previous;
    if (previous.data && previous.data.responseStatus === response && previous.data.eventStatus !== 'cancelled') {
    ...
    if (baseline) {
      return this._saveCheckpoint(key, {
        ...
        outcome: this.RECONCILIATION.OBSERVED_NO_DECISION,
        updatedAt: this._nowIso()
      });
    }
    ...
    if (!mutation.ok) {
      if (this._isSemanticRejection(mutation)) {
        const checkpoint = this._saveCheckpoint(key, {
          responseStatus: response,
          eventStatus: event.status || 'confirmed',
    ...
    return this._saveCheckpoint(key, {
      responseStatus: response,
      eventStatus: event.status || 'confirmed',
      outcome: mutation.data.alreadyApplied
        ? this.RECONCILIATION.ALREADY_RECONCILED
        : this.RECONCILIATION.APPLIED,
      updatedAt: this._nowIso(),
      decision
    }, {
      decisionCandidate: true,
      applied: !!mutation.data.applied
    });
  },

  _saveCheckpoint(key, checkpoint, outcome) {
    const save = CalendarRsvpConfigRepository.saveCheckpoint(key, checkpoint);
    if (!save.ok) return save;
  ...
  _findSecretaryAttendee(event, calendarId, secretaryEmail) {
    ...
    for (let i = 0; i < event.attendees.length; i++) {
      ...
      if (this._normalizeEmail(attendee.email) === normalized) {
        return Result.ok({ responseStatus: String(attendee.responseStatus || 'needsAction') });
      }
    }
  ...
  _applyDecision(event, config, decision) {
    const context = {
      ...
      deployment: { trustedOperatorEmail: config.trustedOperatorEmail },
  ...
  _isSemanticRejection(result) {
    ...
    return code === 'ATTENDANCE_EVENT_AMBIGUOUS' || code === 'INVALID_TRANSITION';
  },
  ...
  handleEventUpdated() {
    ...
    if (!result.ok) {
      const diagnostic = {
        type: 'RSVP_SYNC_FAILURE',
        timestamp: this._nowIso(),
        calendarId: this._configuredCalendarId(),
        resultCode: result.error && result.error.code ? result.error.code : 'UNKNOWN'
      };
  ...
  _configuredCalendarId() {
    try {
      const config = this._readyConfig();
      return config.ok ? config.data.calendarId : '';
    } catch (e) { return ''; }
  }
};
