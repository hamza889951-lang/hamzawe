...
const CalendarRsvpAttendanceService = {
  ...
  RECONCILIATION: { APPLIED: 'APPLIED', ALREADY_RECONCILED: 'ALREADY_RECONCILED', OBSERVED_NO_DECISION: 'OBSERVED_NO_DECISION', SEMANTIC_REJECTION: 'SEMANTIC_REJECTION' },
  initializeBaseline() {
  ...
  _runSyncLocked(config, options) {
    ...
    const checkpointResult = CalendarRsvpConfigRepository.getCheckpoints(); if (!checkpointResult.ok) return checkpointResult;
    ...
    while (true) {
      ...
      const save = CalendarRsvpConfigRepository.saveState(pageResult.data.nextSyncToken, true, processed.data.checkpoints);
      if (!save.ok) return save;
      return Result.ok({ mode: options.baseline ? 'BASELINE' : 'INCREMENTAL', processedEvents: processed.data.processedEvents, decisionCandidates: processed.data.decisionCandidates, applied: processed.data.applied, skipped: processed.data.skipped, semanticRejections: processed.data.semanticRejections, nextSyncToken: pageResult.data.nextSyncToken, restartedFrom410: restartedFrom410 });
    }
  },
  ...
  _processEvents(items, config, checkpoints, baseline) {
    let processedEvents = 0, decisionCandidates = 0, applied = 0, skipped = 0, semanticRejections = 0;
    for (let i = 0; i < items.length; i++) {
      processedEvents++; const outcome = this._processEvent(items[i] || {}, config, checkpoints, baseline); if (!outcome.ok) return outcome;
      decisionCandidates += outcome.data.decisionCandidate ? 1 : 0; applied += outcome.data.applied ? 1 : 0; skipped += outcome.data.skipped ? 1 : 0; semanticRejections += outcome.data.semanticRejection ? 1 : 0;
    }
  ...
  _processEvent(event, config, checkpoints, baseline) {
    ...
    if (event.status === 'cancelled') { checkpoints[key] = { responseStatus: 'cancelled', outcome: 'IGNORED_CANCELLED', updatedAt: this._nowIso() }; return Result.ok({ skipped: true }); }
    ...
    if (attendeeResult.incomplete) return Result.ok({ skipped: true });
    const response = attendeeResult.responseStatus || 'none';
    const previous = checkpoints[key]; if (previous && previous.responseStatus === response) return Result.ok({ skipped: true });
    if (baseline) { checkpoints[key] = { responseStatus: response, outcome: this.RECONCILIATION.OBSERVED_NO_DECISION, updatedAt: this._nowIso() }; return Result.ok({ skipped: true }); }
    const decision = this._decisionForResponse(response);
    if (!decision) { checkpoints[key] = { responseStatus: response, outcome: this.RECONCILIATION.OBSERVED_NO_DECISION, updatedAt: this._nowIso() }; return Result.ok({ skipped: true }); }
    const mutation = this._applyDecision(event, config, decision);
    if (!mutation.ok) {
      if (this._isSemanticRejection(mutation)) {
        checkpoints[key] = { responseStatus: response, outcome: this.RECONCILIATION.SEMANTIC_REJECTION, reasonCode: mutation.error && mutation.error.code ? mutation.error.code : 'UNKNOWN', updatedAt: this._nowIso() };
        return Result.ok({ decisionCandidate: true, semanticRejection: true });
      }
      return mutation;
    }
    checkpoints[key] = { responseStatus: response, outcome: mutation.data.alreadyApplied ? this.RECONCILIATION.ALREADY_RECONCILED : this.RECONCILIATION.APPLIED, updatedAt: this._nowIso() };
  ...
  _findSecretaryAttendee(event, calendarId, secretaryEmail) {
    if (event.attendeesOmitted === true) {
      ...
      if (!fetched.data || fetched.data.attendeesOmitted === true) return { ok: true, incomplete: true };
      event = fetched.data;
    }
    if (!Array.isArray(event.attendees)) return { ok: true, responseStatus: 'none' };
    const normalized = this._normalizeEmail(secretaryEmail);
    for (let i = 0; i < event.attendees.length; i++) {
      const attendee = event.attendees[i] || {};
      if (this._normalizeEmail(attendee.email) === normalized) return { ok: true, responseStatus: String(attendee.responseStatus || 'needsAction') };
    }
    return { ok: true, responseStatus: 'none' };
  },
  _applyDecision(event, config, decision) {
    const context = { operator: { operatorId: config.secretaryEmail }, deployment: { trustedOperatorEmail: config.secretaryEmail }, calendarEvent: { eventId: event.id, calendarId: config.calendarId }, executionPrincipal: this._executionPrincipal() };
  ...
  _isSemanticRejection(result) { const code = result && result.error && result.error.code; return code === 'ATTENDANCE_EVENT_NOT_CORRELATED' || code === 'ATTENDANCE_EVENT_AMBIGUOUS' || code === 'INVALID_TRANSITION' || code === 'ATTENDANCE_EVENT_CORRELATION_LOST'; },
  ...
  handleEventUpdated() { return this.syncNow(); }
};
