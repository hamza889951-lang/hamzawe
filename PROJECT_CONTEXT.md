...
- **M0 (Phase 1.1 — Management Intelligence) — PR #10, CHANGES_REQUESTED → remediation committed, awaiting second architectural review:** Attendance Capture Foundation. New: `Application/AttendanceService.js` (MARK_COMPLETED / MARK_NO_SHOW, explicit only), `Repositories/AttendanceAuditRepository.js` (append-only ATTENDANCE_AUDIT evidence), `AttendanceAddOn.js` (Calendar Add-on surface), `tests/HardeningM0.test.js` (31 tests).
  - **Remediated (architectural review blockers):** `appsscript.json` now declares the Calendar Add-on (`addOns.calendar.eventOpenTrigger.runFunction = onCalendarEventOpen`, `currentEventAccess: "METADATA"`) + explicit `oauthScopes` (verified set incl. `calendar.addons.execute`; no `auth/calendar`, no current-event.read/write). Add-on rewritten on **verified** CardService APIs (`newCardBuilder/newCardHeader.setTitle/addSection/newCardSection.setHeader/addWidget/newTextButton().setText/setOnClickAction/newAction().setFunctionName/setParameters/newActionResponseBuilder().setNavigation(newNavigation().updateCard)`) and the verified event object (`e.calendarEventObject.calendar.{id, calendarId}`; action params via `e.commonEventObject.parameters`; documented legacy fallback for the inherited PoC shape). Operator authorization is now a **derived trust boundary**: identity (Session) + deployment policy (Script Property `ATTENDANCE_OPERATOR_EMAIL`) are separate inputs; the service derives `DOCTOR` authority only on exact match — the entry layer never claims authority.
  - **Evidence status:** local = M0 31/31 + B1–B6 116/116 PASS on the real SlotRepository/Lock/audit stack. Doc-verified = manifest/scope/event-object/CardService contracts (official refs). **Live Google Calendar test deployment NOT yet executed** (no Google toolchain in agent environment; owner-executed per governance) — runbook + 12-item evidence checklist in the PR.
  - Correlation by slot-row `calendar_event_id` (stable id, exactly-one rule; event title is display-only, and in fact unavailable at METADATA). Transitions only via StateMachine inside `SlotRepository.atomicUpdate` (duplicate = deterministic no-op `ALREADY_APPLIED`, zero cell writes; conflicting concurrent decisions cannot both win — `INVALID_TRANSITION` on fresh re-read or `LOCK_TIMEOUT`). Attendance correction (COMPLETED↔NO_SHOW editing) is out of scope — future Attendance Correction Contract.
