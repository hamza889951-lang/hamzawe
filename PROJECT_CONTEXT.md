...
- **M1-C (Phase 1.4 — Capacity & Working-Schedule Intelligence) — CLOSED: approved and merged as PR #14 on 2026-08-25.**
  - Authoritative baseline after M1-C: `0894028ce9d0b450d6a8a4ce049930f3383815da`.
  - Implemented metrics: `CONFIGURED_WORKING_DAYS`, `CONFIGURED_CAPACITY` (with `slotDurationSource` provenance directly from `SettingsRepository.getSlotDurationInfo`), `OBSERVED_WORKING_DAYS`, `OBSERVED_GENERATED_CAPACITY`, `GENERATION_COMPLETENESS` (diagnostic indicator), `BOOKING_UTILIZATION` (doctor-facing commercial KPI with denominator = Configured Capacity).
  - Preserved the governing distinction: `Configured Capacity ≠ Observed Generated Capacity ≠ Bookable Eligibility`.
  - Pure read path (zero writes, zero locks, zero sheet creates); single read per source in `calculateMany`; zero `new Date(` in Application; mixed past periods consistently return `DEFERRED` with `HISTORICAL_NOT_PROVABLE` and `isMixedPeriod: true`.
  - Regression suite: 294/294 PASS (M0=33, B1=14, B2=29, B3=17, B4=13, B5=9, B6=34, M1-A=43, M1-B=50, M1-C=52).
- **M1-B (Phase 1.3 — Report Consumers) — CLOSED: approved and merged as PR #13.** Daily/Weekly/Monthly reports consuming M1-A foundation via single `calculateMany` path; `ReportPeriod` clinic-local calendar boundaries (+03:00); `ReportRenderer` presentation layer; 50/50 tests.
- **M1-A (Phase 1.2 — Metrics Foundation) — CLOSED: approved and merged as PR #12.** Six core foundation metrics (`CONFIRMED_APPOINTMENTS`, `OFFICIAL_CANCELLATIONS`, `OFFICIAL_CHANGES`, `COMPLETED_APPOINTMENTS`, `NO_SHOW_APPOINTMENTS`, `BOOKABLE_SLOTS`), single read per source, 43/43 tests.
