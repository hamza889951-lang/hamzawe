...
const CalendarRsvpConfigRepository = {
  ...
  getRuntimeConfig() {
    try {
      ...
      return Result.fail('RSVP_CONFIG_READ_FAILED', e.message, e.stack);
    }
  },

  getSyncToken() {
    try {
      return Result.ok(String(this._props().getProperty(this.KEYS.SYNC_TOKEN) || '').trim() || null);
    } catch (e) {
  ...
  getInitialized() {
    try {
      return Result.ok(this._props().getProperty(this.KEYS.INITIALIZED) === 'true');
    } catch (e) {
      return Result.fail('RSVP_CHECKPOINT_READ_FAILED', e.message, e.stack);
    }
  },
  ...
  getCheckpoints() {
    try {
      ...
      if (!value || typeof value !== 'object' || Array.isArray(value)) {
        return Result.fail('RSVP_CHECKPOINT_CORRUPT', 'RSVP response checkpoint is not an object');
      }
  ...
  clearSyncToken() {
    try {
      this._props().deleteProperty(this.KEYS.SYNC_TOKEN);
