...
const CalendarRsvpConfigRepository = {
  ...
  setActive(value) {
    try {
      this._props().setProperty(this.KEYS.ACTIVE, value ? 'true' : 'false');
  ...
  validateTrustedOperator(config) {
    if (!config.trustedOperatorEmail) {
      return Result.fail('RSVP_ATTENDANCE_OPERATOR_UNCONFIGURED', 'ATTENDANCE_OPERATOR_EMAIL is required for Calendar RSVP attendance');
    }
    if (config.trustedOperatorEmail !== config.secretaryEmail) {
      return Result.fail('RSVP_OPERATOR_POLICY_MISMATCH', 'ATTENDANCE_OPERATOR_EMAIL must exactly match ATTENDANCE_SECRETARY_EMAIL for RSVP activation', {
        secretaryEmail: config.secretaryEmail,
        trustedOperatorEmail: config.trustedOperatorEmail
      });
    }
  ...
  getCheckpoint(iCalUID) {
    ...
    try {
      ...
      if (!value || typeof value !== 'object' || Array.isArray(value)) {
        return Result.fail('RSVP_CHECKPOINT_CORRUPT', 'RSVP response checkpoint is not an object', { iCalUID });
      }
  ...
  clearSyncState() {
    try {
      const props = this._props();
      props.deleteProperty(this.KEYS.SYNC_TOKEN);
      props.deleteProperty(this.KEYS.INITIALIZED);
      props.deleteProperty(this.KEYS.BOUND_CALENDAR_ID);
      props.deleteProperty(this.KEYS.BOUND_SECRETARY_EMAIL);
