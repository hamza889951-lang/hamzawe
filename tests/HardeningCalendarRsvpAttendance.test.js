...
function makeHarness() {
  ...
  const sandbox = {
    ...
    GoogleSheets: {
      ...
      queryRows: (name, predicate) => {
        const sheet = sheets[name]; if (!sheet) throw new Error('SHEET_NOT_FOUND:' + name);
        return sheet.rows.map((row, idx) => { const obj = { _rowNumber: idx + 2 }; sheet.headers.forEach((h, i) => obj[h] = row[i]); return obj; }).filter(predicate);
      },
...
test('repeated baseline for same binding is rejected', () => { const s = makeHarness(); configure(s); loadRsvp(s); const r = s.CalendarRsvpAttendanceService.initializeBaseline(); assert.strictEqual(r.ok, false); assert.strictEqual(r.error.code, 'RSVP_ACTIVE_REQUIRES_DEACTIVATION'); });
