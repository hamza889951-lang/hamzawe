...
function createSandbox() {
  ...
  sandbox.Calendar = {
    Events: {
      get: function(calendarId, eventId) {
        ...
        return { id: eventId, iCalUID: Object.prototype.hasOwnProperty.call(state.identityMap, eventId)
          ? state.identityMap[eventId] : eventId + '@google.com' };
      }
    }
  };
  ...
  sandbox.CalendarApp = {
    ...
    getDefaultCalendar: function() { return null; }
  };
  ...
  sandbox.Session = {
    getActiveUser: function() {
      return { getEmail: function() { return state.sessionEmail; } };
    }
  };
...
function auditObject(index) {
  ...
  const out = {};
  state.auditHeaders.forEach(function(h, i) { out[h] = row[i]; });
  return out;
}

function cardText(card) { return JSON.stringify(card); }
function sections(card) { return card.widgets || []; }
function sectionByHeader(card, header) { return sections(card).find(function(s) { return s.header === header; }); }
function buttons(section) { return (section && section.widgets || []).filter(function(w) { return w.kind === 'newTextButton' || w.factory === 'newTextButton'; }); }
...
test('M0-ID2 — Add-on event.id alone never equals the stored iCalUID fixture', function() {
  ...
  assert.strictEqual(IC AL_UID_PLACEHOLDER = undefined, undefined);
});
...
test('M0-ID6 — successful Calendar API response without iCalUID fails closed', function() {
  Reset();
  state.identityMode = 'missingIcalUID';
  const result = sandbox.AttendanceService.markCompleted(ctx());
  assert.strictEqual(result.ok, false);
  assert.strictEqual(result.error.code, 'ATTENDANCE_EVENT_IDENTITY_RESOLUTION_FAILED');
  assert.strictEqual(state.availabilityRows[0].status, 'CONFIRMED');
...
test('M0-ID7 — missing calendar context is rejected before API lookup', function() {
  Reset();
  const result = sandbox.AttendanceService.markCompleted(ctx({
    calendarEvent: { eventId: EVENT_ID, calendarId: '' }
  }));
  assert.strictEqual(result.ok, false);
  assert.strictEqual(result.error.code, 'ATTENDANCE_CALENDAR_CONTEXT_INVALID');
...
test('M0-ID8 — ambiguous canonical iCalUID correlation remains rejected', function() {
  Reset();
  state.availabilityRows.push(Object.assign({}, state.availabilityRows[0], {
...
test('M0-ATT2 — existing transition rules remain authoritative', function() {
  Reset();
  sandbox.AttendanceService.markCompleted(ctx());
  const beforeWrites = state.cellWrites;
  const result = sandbox.AttendanceService.markNoShow(ctx());
  assert.strictEqual(result.ok, false);
  assert.strictEqual(result.error.code, 'INVALID_TRANSITION');
  assert.strictEqual(state.availabilityRows[0].status, 'COMPLETED');
  assert.strictEqual(state.cellWrites, beforeWrites);
});
...
test('M0-ATT4 — correlation loss after fresh read remains fail-closed', function() {
  Reset();
  const originalUpdate = sandbox.SlotRepository.atomicUpdate;
  sandbox.SlotRepository.atomicUpdate = function(slotId, decisionFn) {
    state.availabilityRows[0].calendar_event_id = OTHER_ICAL_UID;
    return originalUpdate.call(this, slotId, decisionFn);
  };
  const result = sandbox.AttendanceService.markCompleted(ctx());
  sandbox.SlotRepository.atomicUpdate = originalUpdate;
  assert.strictEqual(result.ok, false);
...
test('M0-UI1 — card uses the requested Arabic attendance buttons and existing handlers', function() {
  Reset();
  const card = sandbox.onCalendarEventOpen({
    calendarEventObject: { calendar: { id: EVENT_ID, calendarId: 'CAL_DEFAULT' } }
  });
  assert.strictEqual(card.title, 'تسجيل حضور الموعد');
  const decisionSection = sectionByHeader(card, 'تسجيل الحضور');
  assert.ok(decisionSection);
  const uiButtons = decisionSection.widgets.filter(function(w) { return w.kind === 'newTextButton'; });
  assert.strictEqual(uiButtons.length, 2);
  assert.deepStrictEqual(uiButtons.map(function(b) { return b.text; }), ['حضر ✅', 'لم يحضر ❌']);
...
test('M0-UI2 — operator email is absent from success and failure cards', function() {
  Reset();
  const success = sandbox.onMarkCompleted({
    commonEventObject: { parameters: { eventId: EVENT_ID, calendarId: 'CAL_DEFAULT' } }
  });
  const successText = cardText(success.navigation.card);
  assert.strictEqual(successText.indexOf(OPERATOR_EMAIL), -1);

  Reset();
  state.identityMode = 'notFound';
  const failure = sandbox.onMarkCompleted({
    commonEventObject: { parameters: { eventId: EVENT_ID, calendarId: 'CAL_DEFAULT' } }
  });
  const failureText = cardText(failure.navigation.card);
  assert.strictEqual(failureText.indexOf(OPERATOR_EMAIL), -1);
});

test('M0-UI3 — failed identity is visibly FAILED and does not look successful', function() {
  Reset();
  state.identityMode = 'notFound';
  const response = sandbox.onMarkNoShow({
    commonEventObject: { parameters: { eventId: EVENT_ID, calendarId: 'CAL_DEFAULT' } }
  });
  const text = cardText(response.navigation.card);
  assert.ok(text.indexOf('FAILED: ATTENDANCE_EVENT_IDENTITY_RESOLUTION_FAILED') !== -1);
  assert.strictEqual(text.indexOf('تم تسجيل عدم الحضور بنجاح'), -1);
  assert.strictEqual(state.availabilityRows[0].status, 'CONFIRMED');
});

test('M0-UI4 — missing event identity renders no decision buttons', function() {
  Reset();
  const card = sandbox.onCalendarEventOpen({ commonEventObject: {} });
  const decisionSection = sectionByHeader(card, 'تسجيل الحضور');
  const uiButtons = decisionSection.widgets.filter(function(w) { return w.kind === 'newTextButton'; });
  assert.strictEqual(uiButtons.length, 0);
});

test('M0-UI5 — UI source keeps internal decision constants/handlers and no email presentation', function() {
  ...
  assert.strictEqual(src.indexOf('operatorId') !== -1 && src.indexOf('Operator:'), true, 'operatorId may remain internal resolution; UI must not render it');
  assert.strictEqual(src.indexOf("Operator: ' +") , -1);
  assert.strictEqual(src.indexOf("AttendanceService.markCompleted"), -1, 'source-inspection string only should not be the user-facing label');
});

test('M0-ARCH1 — AttendanceService uses the existing CalendarRepository boundary and no CalendarApp', function() {
...
test('M0-ARCH2 — GoogleCalendar resolver uses Calendar.Events.get, not suffix manipulation', function() {
  const src = stripComments(fs.readFileSync(path.join(ROOT, 'Infrastructure/GoogleCalendar.js'), 'utf8'));
  assert.ok(src.indexOf('Calendar.Events.get') !== -1);
  assert.strictEqual(src.indexOf("+ '@google.com'"), -1);
  assert.strictEqual(src.indexOf('getEventById(requestedEventId)'), -1);
});

test('M0-ARCH3 — protected state machine and Slot atomic boundary remain present', function() {
